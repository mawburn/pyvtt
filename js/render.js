...
const allow_auto_movement = true
...
function updateViewport() {
  ...
  const speed = interpolation_speed * 1.5
  // move viewport towards desired position
  if (speed > 0) {
    // interpolate viewport position if necessary
    let dx = viewport.newx - viewport.x
    let dy = viewport.newy - viewport.y
...
const ZOOM_MOVE_SPEED = 20.0
const ZOOM_FACTOR_SPEED = 1.05
...
function getPixelData(token, x, y) {
  ...
  // clear memory canvas
  const mem_ctx = mem_canvas.getContext('2d')
  mem_ctx.clearRect(0, 0, mem_canvas.width, mem_canvas.height)

  const dom_canvas = $('#battlemap')[0]
  const sizes = getActualSize(token, dom_canvas.width, dom_canvas.height)
...
const beacons = {} // contains beacons keyed by players' uuid
...
const player_selections = {} // buffer that contains selected tokens and corresponding player colors

let culling = [] // holds tokens for culling
let min_z = -1 // lowest known z-order
let max_z = 1 // highest known z-order
const default_token_size = 60

let background_set = false
...
function drawToken(token, color, is_background) {
  ...
  if (interpolation_speed > 0) {
    if (!(grabbed && select_ids.includes(token.id))) {
      ...
      let dx = token.newx - token.posx
      let dy = token.newy - token.posy
      if (Math.abs(dx) > 20 || Math.abs(dy) > 20) {
        // get normalized direction vector
        const d = Math.sqrt(dx * dx + dy * dy)
        // scale vector
  ...
  const sizes = getActualSize(token, canvas[0].width, canvas[0].height)
  ...
  if (is_background) {
    ...
    if (token.text !== null) {
      if (token.label_canvas === null) {
        ...
        const padding = 14 // to allow for line width
        const width = metrics.width + padding
        const height = (fontsize * 96) / 72 + padding
      ...
      try {
        const left = -token.label_canvas.width / 2
        let top = token.label_canvas.height / 4
        if (is_timer) {
          // vertical center it
          top = -token.label_canvas.height / 2
        }
        const scale = Math.sqrt(1 / viewport.zoom) / 2.0
...
let last_update = 0 // timestamp with integer ms
let num_frames = 0 // number of frames since `last_update` was reset

const running = false // main loop running?

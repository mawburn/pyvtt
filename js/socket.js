...
let socket = null // websocket used for client-server-interaction
const quiet = true

const ping_delay = 2000 // delay between two pings
let next_ping = null // indicates when the next ping will be sent
let last_ping = null // timestamp when last ping was sent

const socket_move_delay = 100
const socket_move_timeout = 0 // delay until the next move update will be sent through the socket

// --- game state implementation ----------------------------------------------

let game_url = ''
let gm_name = ''
const gm_dropdown = false // GM's scenes view
const history_dropdown = false // roll history
const timeid = 0
const full_update = true
const scene_id = 0
let is_gm = false
...
function onCountdown(hexstamp) {
  ...
  if (s > 0) {
    ...
    const line1 = 'GAME STARTS AT<br /><h2>' + start + '</h2>'
    let line2 = '<h1>'
    if (d > 0) {
      line2 += d + '<span class="small">D</span>'
    }
    if (d > 0 || h > 0) {
      line2 += h + '<span class="small">H</span>'
    }
    if (d > 0 || h > 0 || m > 0) {
      line2 += m + '<span class="small">M</span>'
    }
    if (d > 0 || h > 0 || h > 0 || s > 0) {
      line2 += s + '<span class="small">S</span></h1>'
    }

    $('#countdown')[0].innerHTML = line1 + line2

    window.setTimeout(function () {
      onCountdown(hexstamp)
    }, 1000)
  } else {

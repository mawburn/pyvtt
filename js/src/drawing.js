...
function getDoodlePos(event) {
  ...
  let x = (event.clientX - box.left) / box.width
  let y = (event.clientY - box.top) / box.height
...
function onMovePen(event) {
  ...
  // grab relevant data
  const width = detectPressure(event)
  const pos = getDoodlePos(event)
  ...
  if (event.buttons == 1 || event.type == 'touchstart' || event.type == 'touchmove') {
    ...
    if (drag != null) {
      ...
      // add segment
      const line = new Line(drag[0], drag[1], pos[0], pos[1], width, color)
      edges.push(line)
    }
...
function onReleasePen(event) {
  ...
  // start new line
  straight = new Line(pos[0], pos[1], null, null, width, color)
}

/// Modify line width using the mouse wheel
function onWheelPen(event) {
  let pressure = parseInt(localStorage.getItem('draw_pressure'))
  if (event.deltaY < 0) {
    pressure += 3
    if (pressure >= 100) {
      pressure = 100
